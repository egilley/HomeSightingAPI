...
router.get('/commonName/:id', function(req, res, next) {
  var id = (req.params.id); // will contains data from :id
   ...
   // res.send(result); // send the data
   var item =  jsonFile.find(u => u.commonName == id)
    //res.render('lepidopteraSingle.jade', { lepidopteraList: jsonFile.find(u => u.commonName == id)});
    res.render('lepidopteraSingle.jade', { lepidopteraList: item});
  // etc ...
});
